---
summary: "OpenClaw에서 OpenAI를 API 키 또는 Codex 구독으로 사용"
read_when:
...
OpenAI는 GPT 모델에 대한 개발자 API를 제공합니다. Codex는 구독 접근을 위한 **ChatGPT 로그인** 또는 사용량 기반 접근을 위한 **API 키** 로그인을 지원합니다.
...
- 모델 참조는 항상 `provider/model`을 사용합니다 ([모델](/ko-KR/concepts/models) 참조).
